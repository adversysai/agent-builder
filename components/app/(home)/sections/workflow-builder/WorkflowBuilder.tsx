...
/**
 * Sanitize node data to ensure all fields are valid and remove React elements or invalid objects
 */
function sanitizeNodeData(data: any): any {
  ...
  for (const [key, value] of Object.entries(data)) {
    // Skip React elements and internal React properties
    if (
      value &&
      typeof value === 'object' &&
    ...
    // Skip 'label' field - it's generated by createNodeLabel() and shouldn't be in data
    if (key === 'label') {
      continue; // Skip label - it will be regenerated
    }

    // Handle specific fields that must be strings
    if (key === 'nodeName' || key === 'name') {
    ...
    if (Array.isArray(value)) {
      sanitized[key] = value.map((item) => {
        if (typeof item === 'object' && item !== null) {
          if ((item as any).$$typeof === Symbol.for('react.element')) {
            return null; // Remove React elements
          }
          // Recursively sanitize objects in arrays
          return sanitizeNodeData(item);
        }
        return item;
      }).filter((item) => item !== null); // Remove null React elements
      continue;
    }
...
function WorkflowBuilderInner({ onBack, initialWorkflowId, initialTemplateId }: WorkflowBuilderProps) {
  ...
  const [showFixWithAI, setShowFixWithAI] = useState(false);
  const [nodeForFix, setNodeForFix] = useState<Node | null>(null);
  ...
  const { runWorkflow, stopWorkflow, isRunning, nodeResults, execution, currentNodeId, pendingAuth, resumeWorkflow, rerunNode } = useWorkflowExecution();
  ...
  return (
    ...
    >
      ...
      >
        ...
        {showTestEndpoint && workflow ? (
          ...
          <ExecutionPanel
            ...
            onAnalyzeResults={(exec, results) => {
              setShowAIChat(true);
              // Execution results will be passed via latestExecution prop
            }}
            onSendNodeToAI={(nodeId, result) => {
              setShowAIChat(true);
              // Could set a specific node context here if needed
            }}
          ...
          <LogicNodePanel
            ...
            error={(() => {
              const nodeResult = nodeResults[selectedNode.id];
              if (nodeResult?.error || nodeResult?.status === 'failed') {
                return {
                  message: nodeResult.error || nodeResult.message || 'Node execution failed',
                  type: nodeResult.errorType || 'execution',
                };
              }
              if (execution?.status === 'failed' && execution.error && execution.error.includes(selectedNode.id)) {
                return {
      ...
      {/* Fix With AI Panel */}
      {showFixWithAI && nodeForFix && (
        <FixWithAIPanel
          node={nodeForFix as any}
          workflow={{
            nodes: nodes as any,
            edges: edges as any,
            name: workflow?.name,
          ...
          error={(() => {
            // Extract error for this specific node
            const nodeResult = nodeResults[nodeForFix.id];
            if (nodeResult?.error || nodeResult?.status === 'failed') {
              return {
                message: nodeResult.error || nodeResult.message || 'Node execution failed',
                type: nodeResult.errorType || 'execution',
            ...
            if (execution?.status === 'failed' && execution.error && execution.error.includes(nodeForFix.id)) {
              return {
                message: execution.error,
                type: 'workflow',
              };
          ...
          onClose={() => {
            setShowFixWithAI(false);
            setNodeForFix(null);
          }}
          onApplyFix={(fixedNode) => {
            // Sanitize the fixed node data to ensure all fields are valid
            const sanitizedData = sanitizeNodeData(fixedNode.data);
            handleUpdateNodeData(fixedNode.id, sanitizedData);
            setShowFixWithAI(false);
            setNodeForFix(null);
          }}
        />
